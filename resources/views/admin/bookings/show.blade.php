...
@section('content')
<div class="mb-6">
    <a href="{{ route('admin.bookings.index') }}" class="text-sm text-red-600 hover:underline">← Kembali</a>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            ...
            <div class="grid grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <a href="{{ route('admin.properties.show', $booking->room->property) }}"
                       class="text-red-600 hover:underline">{{ $booking->room->property->name }}</a>
                </div>
                <div>
    ...
    <div class="space-y-5">
        ...
        {{-- User --}}
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
            <h3 class="font-semibold text-gray-700 mb-3">Dipesan Oleh</h3>
            <div class="text-sm space-y-2">
                <p class="font-medium text-gray-700">{{ $booking->user->name }}</p>
                <p class="text-gray-500">{{ $booking->user->email }}</p>
                <a href="{{ route('admin.users.show', $booking->user) }}"
                   class="text-red-600 hover:underline text-xs">Lihat profil user</a>
            </div>
        </div>
